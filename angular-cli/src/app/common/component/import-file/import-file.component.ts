...
export class ImportFileComponent extends BaseComponent implements OnInit {
  ...
  @Input() accept?: string = '.xlsx';
	...
	@Input() bigInitUrl: string = '/system-management/api/v1/system/upgrade/upload/notice';
  ...
  ngOnInit(): void {
	...
	nzCustomBigReq = (item: NzUploadXHRArgs) => {
		...
		return this.http.post<any>(this.bigInitUrl, {
				...
				.subscribe(
					...
					() => {
						if (this.uploadRes.body && this.uploadRes.body.data.filePath) {
							item.onSuccess!(this.uploadRes.body, item.file!, this.uploadRes);
						};
					}
				);
		},(err) => {
